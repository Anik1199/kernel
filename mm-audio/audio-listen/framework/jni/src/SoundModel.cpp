                 ...
                 jobject                   soundModelInfo)  // ListenTypes$SVASoundModelInfo
{
   ...
   char **      keywordPhrases = NULL;
   char **      userNames = NULL;
// %%% HARDCODED - using Maximum keywords and max users as 10
   jstring users[10];
   jstring keywords[10];
   ...
   if (kSucess != status) {
      ...
      ALOGE("getSMInfoV2: ERROR getSoundModelHeader() returned numKWs zero");
      eStatus = LISTEN_EFAILURE;
      goto cleanup_for_getSMInfoV2;
   ...
   ALOGD("getSMInfoV2: numKWs = %d, numUsers = %d, numPairs = %d", numKWs, numUsers, numActivePairs);
   ...
   ALOGD("getSMInfoV2: getKeywordPhrase output %d KW strings", numKWs );
   ...
   if (numUsers > 0) {
      eStatus = allocStrArr(numUsers, &userNames);
      if (eStatus != LISTEN_SUCCESS) {
         ALOGE("getSMInfoV2: ERROR could not alloc memory for String arrayu");
         goto cleanup_for_getSMInfoV2;
      }
      status = ListenSoundModelLibV2::getUserNames(pModel, &numUsers, userNames);
      if (kSucess != status) {
   ...
   kwInfoArr = env->NewObjectArray(numKWs, kwInfoClazz, tmpKWInfo);
   ...
   ALOGD("getSMInfoV2: fill %d KWs", numKWs);
   kwInfoConstructorId   = env->GetMethodID(kwInfoClazz, "<init>", "()V");
   for (int kw=0; kw < numKWs; kw++) {
